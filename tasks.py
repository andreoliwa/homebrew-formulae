...
def choose_formula(c) -> str:
    ...
    rv = c.run(
        r"fd .rb Formula/ | rg -o 'Formula/(.+)\.rb' -r '$1' | fzf --reverse --height 40% --select-1"
    ).stdout.strip()
    if not rv:
        raise Exit()
    return rv
...
@task(
    help={
        "interactive": "Install the package and open an interactive shell to test it",
...
def brew_install(c, interactive=False, head=False, debug=False, test_only=False):
    """Install a Homebrew formula. Detect formula and user name from the current dirs."""
